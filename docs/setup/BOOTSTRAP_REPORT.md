...
| Product implementation    | G0 data-truth foundation implemented (domain contracts, SQL truth + migrations, persistence repos, evidence index, object store, DR drill harnesses) and CI-gated; see `README.md` and `specs/g0-contracts-data-truth/spec.md`. Later milestones not started — check `pnpm autopilot:status` and `specs/implementation/**` for live truth.                           |
