...
CREATE TABLE col.collector_incidents (
    ...
    kind              text NOT NULL CHECK (kind IN (
                          ...
                          'PARITY_FAILURE')),
    decoder_id        text NOT NULL CHECK (length(decoder_id) > 0),
    ...
    CONSTRAINT collector_incidents_scope_key UNIQUE (
        incident_id, decoder_id, program_id, program_version),
...
CREATE INDEX collector_incidents_scope_status_idx
    ON col.collector_incidents (decoder_id, program_id, program_version, status);
...
CREATE TABLE col.collector_decode_pauses (
    ...
    FOREIGN KEY (opening_incident_id, decoder_id, program_id, program_version)
...
CREATE UNIQUE INDEX collector_decode_pauses_active_scope_idx
    ON col.collector_decode_pauses (decoder_id, program_id, program_version)
