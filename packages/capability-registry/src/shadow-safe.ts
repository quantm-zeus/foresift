 ...
 * package is imported. The snapshot boundary additionally reads ALL own string
 * keys (not only enumerable ones) and uses a captured `Array.isArray`, so a
 * non-enumerable own field is materialized and a shadowed `Array.isArray`
 * cannot collapse the snapshot.
 */
