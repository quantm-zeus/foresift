...
function fromRow(row: Record<string, unknown>): CollectorScopeDeclaration {
  return CollectorScopeDeclarationSchema.parse({
    ...
    accountFilters: row.account_filters,
    coverageStart:
    ...
    byteEnvelope: Number(row.byte_envelope),
    quotaEnvelope: Number(row.quota_envelope),
    maxLagSlots: Number(row.max_lag_slots),
    maxGapAgeMs: Number(row.max_gap_age_ms),
    rightsPolicyRef: row.rights_policy_ref,
    retentionPolicyRef: row.retention_policy_ref,
    supportManifestRef: row.support_manifest_ref,
    active: row.active,
  });
}
export class CollectorScopeLoader {
  ...
  async loadActive(): Promise<readonly CollectorScopeDeclaration[]> {
...
export class ScopeSubscriptionBuilder {
  private readonly byId = new Map<string, CollectorScopeDeclaration>();
  constructor(
  ...
  build(scopeId: string, request: SubscriptionRequest): ScopedSubscription {
    ...
    const allowed =
      ...
      scope.eventFamilies.includes(request.eventFamily) &&
