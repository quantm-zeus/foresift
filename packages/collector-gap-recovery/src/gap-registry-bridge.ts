...
export class GapRegistryBridge {
  ...
  async transition(
    ...
    const r = await this.engine.query<{ status: GapLifecycle }>(
      'SELECT status FROM col.collector_gap_registry WHERE gap_id=$1',
      [gapId],
    );
    ...
    if (
      ...
      (!waiver || Date.parse(waiver.expiresAt) <= this.now().getTime())
    )
      throw new Error('VALID_SIGNED_EXPIRING_WAIVER_REQUIRED');
    await this.engine.query(
      'UPDATE col.collector_gap_registry SET status=$2,waiver_scope=$3,waiver_signature=$4,waiver_expires_at=$5,updated_at=$6 WHERE gap_id=$1',
      [
        gapId,
        to,
        waiver?.scope ?? null,
