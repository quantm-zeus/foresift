...
export function evaluateDrift(input: DriftAssessment): DriftResult {
  const detected =
    Number.isFinite(input.statistic) &&
    Number.isFinite(input.registeredThreshold) &&
    Math.abs(input.statistic) >= input.registeredThreshold;
  const utilitySensitive =
    input.kind === DriftControlKind.CALIBRATION_DRIFT ||
    input.kind === DriftControlKind.REGIME_DRIFT;
  const fraction =
    detected && utilitySensitive ? Math.min(1, Math.max(0, input.degradationFraction)) : 0;
