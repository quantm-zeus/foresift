...
export function registerExperiment(input: RegisteredExperiment): Readonly<RegisteredExperiment> {
  if (
    !input.hypothesis ||
    !input.sequentialRuleId ||
    (input.resultsAvailableAt !== null &&
      Date.parse(input.registeredAt) >= Date.parse(input.resultsAvailableAt))
  )
    throw new EvalError(
      'experiment must be registered before results',
...
): readonly (HypothesisResult & {
  readonly adjustedPValue: number;
  readonly winnersCurseRisk: boolean;
})[] {
  ...
  const adjusted = ordered.map((result, index) => {
    ...
    return {
      ...
      winnersCurseRisk:
        index === 0 &&
        count > 1 &&
        Math.abs(result.observedEffect) ===
          Math.max(...ordered.map((item) => Math.abs(item.observedEffect))),
    };
  });
...
export function validatePowerPrecisionPlan(plan: PowerPrecisionPlan): Readonly<PowerPrecisionPlan> {
  const counts = [
    plan.minimumMatureCases,
    plan.minimumSuccesses,
    plan.minimumFailures,
    plan.minimumRugs,
    plan.minimumClusterEss,
    plan.minimumCalendarBlocks,
  ];
  if (
    !plan.version ||
    counts.some((value) => !Number.isFinite(value) || value <= 0) ||
    plan.requiredRegimes.length === 0 ||
    plan.maximumIntervalWidth <= 0 ||
    plan.minimumDetectableUtility <= 0
  )
    throw new EvalError(
      'power plan must specify every precision dimension',
      { version: plan.version },
      ErrorCode.EVAL_ESS_BELOW_GATE,
    );
