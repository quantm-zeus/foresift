...
export function freezeReplayManifest(
  ...
  if (input.networkAccess === true)
    throw new EvalError('decision-quality replay network access is denied', { replayId: input.replayId }, ErrorCode.EVAL_NETWORK_ACCESS_DENIED);
  if (Date.parse(input.observationCutoff) > Date.parse(input.createdAt))
    throw new EvalError('observation cutoff exceeds manifest freeze time', { replayId: input.replayId }, ErrorCode.EVAL_UNIVERSE_MISMATCH);
...
export function runFrozenReplay<T>(input: {
  ...
  if (input.networkAccess === true || input.manifest.networkAccess !== false)
    throw new EvalError('frozen replay cannot access a network', { replayId: input.manifest.replayId }, ErrorCode.EVAL_NETWORK_ACCESS_DENIED);
  ...
  const records = input.records
    .filter((record) => {
      ...
      if (!Number.isFinite(available))
        throw new EvalError('input availability is invalid', { stableId: record.stableId }, ErrorCode.EVAL_UNIVERSE_MISMATCH);
...
export function assertSingleReplayArtifactClass(artifacts: readonly FrozenReplayArtifact[]): ReplayKind {
  const kinds = new Set(artifacts.map((artifact) => artifact.artifactClass));
  if (kinds.size !== 1)
    throw new EvalError('replay artifact classes cannot be blended', {}, ErrorCode.EVAL_UNIVERSE_MISMATCH);
  const kind = artifacts[0]?.artifactClass;
  if (!kind)
    throw new EvalError('at least one replay artifact is required', {}, ErrorCode.EVAL_UNIVERSE_MISMATCH);
  return kind;
}
