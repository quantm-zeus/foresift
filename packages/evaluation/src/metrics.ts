...
function ndcgAtK(candidates: readonly MetricCandidate[], k: number): number | null {
  ...
  const dcg = top.reduce((sum, item, index) =>
    sum + (item.eligibleGem ? 1 / Math.log2(index + 2) : 0), 0);
  const relevant = Math.min(k, candidates.filter((item) => item.eligibleGem).length);
  const ideal = Array.from({ length: relevant }, (_, index) => 1 / Math.log2(index + 2))
...
export function computeMetricSuite(input: MetricSuiteInput): MetricSuiteResult {
  if (!Number.isInteger(input.k) || input.k <= 0 || !input.populationClaim)
    throw new EvalError('invalid metric scope', { k: input.k }, ErrorCode.EVAL_POPULATION_CLAIM_UNSUPPORTED);
  assertCompleteDenominatorDisclosure(input.denominatorDisclosure);
  if (input.final && input.candidates.some((candidate) => candidate.maturityState !== MaturityState.FULLY_MATURED))
    throw new EvalError('FINAL metrics require fully matured valid outcomes', {}, ErrorCode.EVAL_POPULATION_CLAIM_UNSUPPORTED);
  ...
  const returns = ordered.flatMap((candidate) => candidate.netReturn === undefined ? [] : [candidate.netReturn]);
  const totalCapital = input.candidates.reduce((sum, candidate) => sum + (candidate.capitalCapacity ?? 0), 0);
  const deployed = input.candidates.reduce((sum, candidate) => sum + (candidate.capitalDeployed ?? 0), 0);
  ...
  const values: Record<string, number | null> = {
    ...
    FALSE_REJECTION_RATE: ratio(input.candidates.filter((candidate) => !candidate.predictedPositive && candidate.eligibleGem).length, positives),
    MEDIAN_SUCCESSFUL_ASSET_RANK: median(successes.map((candidate) => candidate.rank)),
    MEDIAN_ACTIONABLE_LEAD_TIME: median(input.candidates.flatMap((candidate) => candidate.actionableLeadTimeMs === undefined ? [] : [candidate.actionableLeadTimeMs])),
    MFE: average(input.candidates.flatMap((candidate) => candidate.mfe === undefined ? [] : [candidate.mfe])),
    MAE: average(input.candidates.flatMap((candidate) => candidate.mae === undefined ? [] : [candidate.mae])),
    TARGET_DURATION: average(input.candidates.flatMap((candidate) => candidate.targetDurationMs === undefined ? [] : [candidate.targetDurationMs])),
    LIQUIDITY_SURVIVAL: ratio(input.candidates.filter((candidate) => candidate.liquiditySurvived).length, input.candidates.filter((candidate) => candidate.liquiditySurvived !== undefined).length),
    SECURITY_SURVIVAL: ratio(input.candidates.filter((candidate) => candidate.securitySurvived).length, input.candidates.filter((candidate) => candidate.securitySurvived !== undefined).length),
    TRADABLE_SUCCESS_BY_NOTIONAL: ratio(tradable.length, input.candidates.length),
    FILL_EXIT_SURVIVAL: ratio(input.candidates.filter((candidate) => candidate.filled && candidate.exited).length, input.candidates.filter((candidate) => candidate.filled !== undefined).length),
    PARTIAL_FILL_RATE: ratio(input.candidates.filter((candidate) => candidate.partialFill).length, input.candidates.filter((candidate) => candidate.partialFill !== undefined).length),
    SIGNAL_TO_TRADABLE_DIVERGENCE: ratio(signal.filter((candidate) => candidate.tradableSuccess !== true).length, signal.length),
    OUTCOME_MATURITY_RATE: ratio(input.denominatorDisclosure.fullyMaturedValidCount, input.denominatorDisclosure.eligibleCount),
    OUTCOME_CENSORING_RATE: ratio(input.denominatorDisclosure.counts.CENSORED, input.denominatorDisclosure.eligibleCount),
    OUTCOME_INVALID_DATA_RATE: ratio(input.denominatorDisclosure.counts.INVALID_DATA, input.denominatorDisclosure.eligibleCount),
    EXECUTABLE_TARGET_FALSE_POSITIVE_RATE: ratio(input.candidates.filter((candidate) => candidate.executableTargetFalsePositive).length, input.candidates.length),
