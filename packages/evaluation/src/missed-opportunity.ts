...
import { candidateActionTime, type CandidateActionTime, type EvaluationArm } from './action-time.ts';
...
export interface MissedOpportunityInput {
  ...
  readonly evidenceDecision: 'REQUESTED' | 'NOT_REQUESTED' | 'COST_BLOCKED' | 'QUOTA_BLOCKED' | 'CAPABILITY_UNAVAILABLE';
...
function classify(input: MissedOpportunityInput): MissClassification {
  ...
  if (!input.coverageExists || input.discoveredAt === null) return input.collectorGap ? 'COLLECTOR_GAP' : 'NOT_DISCOVERED';
...
export function analyzeMissedOpportunity(input: MissedOpportunityInput): MissedOpportunityAnalysis {
  if (!input.nextEligibleDatasetId || input.nextEligibleDatasetId === input.currentHoldoutId)
    throw new EvalError('miss analysis must attach only to a next eligible dataset', { candidateId: input.candidateId }, ErrorCode.EVAL_HOLDOUT_EXHAUSTED_REUSED);
  if (input.frozenEvidenceRefs.length === 0 || Object.keys(input.frozenVersions).length === 0)
    throw new EvalError('miss analysis requires frozen evidence and versions', { candidateId: input.candidateId }, ErrorCode.EVAL_UNIVERSE_MISMATCH);
  ...
  const sourceAt = input.firstSourceAvailableAt === null ? null : Date.parse(input.firstSourceAvailableAt);
  const sourceDelayMs = discoveredAt === null || sourceAt === null ? null : discoveredAt - sourceAt;
  const decisionDelayMs = discoveredAt === null
    ? null
  ...
  const steps = [
    ...
    { step: 2, name: 'FIRST_SOURCE_AVAILABILITY', result: input.firstSourceAvailableAt ?? 'UNOBSERVED' },
    { step: 3, name: 'FUNNEL_EXIT', result: input.funnelExit ?? 'NO_RECORDED_EXIT' },
    { step: 4, name: 'CLASSIFICATION', result: classification },
    { step: 5, name: 'DELAY_DECOMPOSITION', result: `${sourceDelayMs ?? 'NA'}:${decisionDelayMs ?? 'NA'}` },
