...
export function diagnoseSelectionBias(input: {
  ...
  const propensityValid = input.assignments.every((assignment) => {
    ...
    const maturity =
      assignment.outcomeMaturedAt === null
        ? Number.POSITIVE_INFINITY
        : Date.parse(assignment.outcomeMaturedAt);
    return (
      Number.isFinite(assigned) &&
      Number.isFinite(retrieval) &&
      assigned <= retrieval &&
      assigned < maturity &&
      Number.isFinite(assignment.assignmentProbability) &&
  ...
  const claimRestriction =
    propensityValid && positivity && overlap && weightStable
