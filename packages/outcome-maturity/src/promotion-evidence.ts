...
export type ModuleMaturity = 'NOT_IMPLEMENTED' | 'IMPLEMENTED' | 'AVAILABLE' | 'SHADOW' | 'PROVEN';
...
export function evaluatePromotionEvidence(
  ...
  const highResolution =
    input.evidenceResolution === EvidenceResolution.HIGH_RESOLUTION_EXECUTION ||
    input.evidenceResolution === EvidenceResolution.EXACT_CONFIGURATION_EXECUTION;
  const moduleEligible = moduleSupportsClaims(
    input.moduleMaturity,
    input.profileRequiresProvenModule,
  );
  ...
  const capacityComplete =
    !input.capacityLimited ||
    (input.capacityEvidence !== null &&
      input.capacityEvidence !== undefined &&
  ...
  const largerThanProven =
    input.capacityLimited && input.capacityEvidence
      ? Number(input.required.notional) > Number(input.capacityEvidence.maximumExecutableNotional)
      : false;
  const capacityEligible =
    capacityComplete && (!largerThanProven || Boolean(input.largerNotionalSimulationRef));
  const exactMatureTradable =
    simulation.tradableLabel === PromotionOutcomeLabel.TRADABLE_SUCCESS &&
    simulation.outcomeMaturity === MaturityState.FULLY_MATURED &&
    highResolution &&
    exactConfigurationMatch;
  const promotionEligible =
    exactMatureTradable && moduleEligible && !postExpiryExcluded && capacityEligible;
  ...
  const record: PromotionEvidenceRecord = {
    ...
    outcomeLabel: (simulation.tradableLabel ??
      PromotionOutcomeLabel.PENDING) as PromotionEvidenceRecord['outcomeLabel'],
    ...
    optimisticSensitivity: ordering.pathAmbiguous ? (input.optimisticSensitivity ?? null) : null,
    ...
    totalDeployablePortfolioCapacity:
      input.capacityEvidence?.totalDeployablePortfolioCapacity ?? null,
...
export function requirePromotionEvidence(input: PromotionEvidenceInput): PromotionEvidenceRecord {
  ...
  if (!evaluation.promotionEligible) {
    const capacityFailure = evaluation.refusalReasons.includes(
      'CAPACITY_DISCLOSURE_OR_LARGER_SIMULATION_REQUIRED',
    );
    throw new MatError(
