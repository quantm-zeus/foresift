...
describe('Promotion Evidence (FR-MAT-008…012, AC-152)', () => {
  it('approves promotion only on exact configuration match with fully matured high-res evidence', () => {
    const matchCase = GOLDEN_PROMOTION_CONFIG_CASES.find((c) => c.caseId === 'promo_exact_match_success');
    expect(matchCase?.supportsPromotion).toBe(true);

    const mismatchCase = GOLDEN_PROMOTION_CONFIG_CASES.find((c) => c.caseId === 'promo_mismatch_notional');
    ...
    const coarseCase = GOLDEN_PROMOTION_CONFIG_CASES.find((c) => c.caseId === 'promo_coarse_only_refusal');
    expect(coarseCase?.supportsPromotion).toBe(false);
    expect(coarseCase?.refusalReason).toBe('COARSE_SIGNAL_CANNOT_SUBSTITUTE_FOR_HIGH_RES_EXECUTION');
  });

  it('enforces adverse ordering primacy when timestamps within coarse interval are ambiguous (FR-MAT-009)', () => {
    const ambiguousCase = GOLDEN_PATH_ORDERING_CASES.find((c) => c.caseId === 'path_ambiguity_adverse_primacy');
  ...
  it('excludes post-expiry gains from actionable success (FR-MAT-011)', () => {
    const expiryCase = GOLDEN_EXPIRY_CASES.find((c) => c.caseId === 'expiry_post_window_pump_excluded');
    expect(expiryCase?.actionableSuccessRecorded).toBe(false);
    expect(expiryCase?.gainExcludedReason).toBe('POST_EXPIRY_GAINS_EXCLUDED_FROM_ACTIONABLE_SUCCESS');
  });
