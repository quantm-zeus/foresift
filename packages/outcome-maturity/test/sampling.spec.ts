...
describe('Sampling and Horvitz-Thompson Estimation (FR-MAT-007, AC-128)', () => {
  ...
  it('detects extreme weight violations and restricts population claims', () => {
    const extremeScenario = GOLDEN_SAMPLING_VECTORS.find(
      (s) => s.scenarioId === 'sampling_extreme_weight_violation',
    );
    ...
    expect(extremeScenario.requiredClaimLimitation).toBe(
      'SAMPLE_STRATUM_COVERAGE_ONLY_WEIGHT_STABILITY_REFUSED',
    );
  });
});
