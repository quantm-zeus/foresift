...
describe('migration suite shape (+AC-243 probe assignments)', () => {
  it('discovers exactly the G0 scripts in lexicographic order', async () => {
    ...
    expect(migrations.map((m) => m.id)).toEqual([
      ...
      // `prov` sorts lexicographically BETWEEN `dr` and `sec` — the manifest-
      // declared g0_prov_* scripts land inside the existing family order.
...
describe('applyMigrations (FR-DATA-001…006, FR-DR-001/002 foundation)', () => {
  ...
  it(
    ...
    async () => {
      ...
      expect(report.applied.length).toBe(21);
  ...
  it('applies twice without damage (idempotent)', async () => {
    ...
    expect(second.skipped.length).toBe(21);
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        expect((await appliedMigrations(engine)).length).toBe(21);
