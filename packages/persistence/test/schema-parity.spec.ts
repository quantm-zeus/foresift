...
describe('Drizzle mirror parity with SQL truth (ADR-001)', () => {
  ...
  it('matches every column: name, nullability, and type class', async () => {
    ...
    for (const entry of Object.values(mirror)) {
      ...
      if (sqlCols.rows.length === 0) {
        failures.push(`${schema}.${config.name}: exists in mirror but not in SQL`);
        continue;
      }
  ...
  it('matches primary keys on every table', async () => {
    ...
    const sqlPkMap = new Map(sqlPks.rows.map((r) => [`${r.table_schema}.${r.table_name}`, [...r.pk_cols].sort()]));
    ...
    for (const entry of Object.values(mirror)) {
      ...
      const expected = sqlPkMap.get(`${schema}.${config.name}`);
      expect(expected, `${schema}.${config.name} missing PK in SQL`).toBeDefined();
      expect([...mirrorPk].sort(), `${schema}.${config.name} PK mismatch`).toEqual(expected!);
    }
  });
});
