...
describe('Drizzle mirror parity with SQL truth (ADR-001)', () => {
  it('mirrors exactly the table set created by the migrations', async () => {
    const sqlTables = await engine.query<{ table_schema: string; table_name: string }>(
      `SELECT table_schema, table_name FROM information_schema.tables
       WHERE (table_schema = 'public' OR table_schema = 'sig')
    ...
    const mirrorNames = Object.values(mirror)
      .map((v) => {
        ...
        return config.schema && config.schema !== 'public'
  ...
  it('matches every column: name, nullability, and type class', async () => {
    ...
    for (const entry of Object.values(mirror)) {
      ...
      const qualifiedName = schemaName === 'public' ? config.name : `${schemaName}.${config.name}`;
      ...
      if (sqlCols.rows.length === 0) {
        failures.push(`${qualifiedName}: exists in mirror but not in SQL`);
        continue;
      }
      ...
      for (const col of config.columns) {
        ...
        if (!sqlCol) {
          failures.push(`${qualifiedName}.${col.name}: in mirror but missing in SQL`);
          continue;
        }
        const expectedClass = TYPE_CLASS[sqlCol.data_type];
        if (expectedClass === undefined) {
          failures.push(`${qualifiedName}.${col.name}: unmapped SQL type ${sqlCol.data_type}`);
        } else if (col.dataType !== expectedClass) {
          failures.push(
            `${qualifiedName}.${col.name}: mirror type ${col.dataType} != SQL ${sqlCol.data_type} (${expectedClass})`,
          );
        }
        if (col.notNull !== (sqlCol.is_nullable === 'NO')) {
          failures.push(
            `${qualifiedName}.${col.name}: nullability mismatch (mirror notNull=${String(col.notNull)}, SQL=${sqlCol.is_nullable})`,
          );
        }
      }
      for (const [name] of byNameSql) {
        if (!byNameMirror.has(name)) {
          failures.push(`${qualifiedName}.${name}: in SQL but missing in mirror`);
        }
      }
    }
  ...
  it('matches primary keys on every table', async () => {
    const sqlPks = await engine.query<{ table_schema: string; table_name: string; pk_cols: string[] }>(
      `SELECT tc.table_schema,
       ...
       WHERE (tc.table_schema = 'public' OR tc.table_schema = 'sig')
    ...
    for (const entry of Object.values(mirror)) {
      ...
      const expected = sqlPkMap.get(qualifiedName);
      expect(expected, `${qualifiedName} missing PK in SQL`).toBeDefined();
      expect([...mirrorPk].sort(), `${qualifiedName} PK mismatch`).toEqual(expected!);
    }
  });
});
