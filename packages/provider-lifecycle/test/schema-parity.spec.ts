/**
 * ADR-001 conformance for the provider schema namespace: the Drizzle mirror
 * must match SQL truth. The migrations are the source of truth; this test
 * enumerates every `prov`-schema table in `information_schema` and compares
 * columns, nullability, type classes, and primary keys against the generated
 * mirror (same arrangement as the proven persistence and security packages,
 * scoped to the `prov` schema — see plan.md material decision 1).
 */
...
const MIRROR_TABLE_COUNT = Object.values(mirror).filter((v) => asTable(v) !== undefined).length;
...
describe('prov-schema Drizzle mirror parity with SQL truth (ADR-001)', () => {
  it('applies all five g0_prov migrations over the full proven set', async () => {
    ...
    // The proven data/dr/sec families stay applied alongside — one migration truth.
  ...
  it('adds zero tables to the public schema (proven parity contract untouched)', async () => {
    const publicTables = await engine.query<{ table_name: string }>(
      `SELECT table_name FROM information_schema.tables
       WHERE table_schema = 'public' AND table_name LIKE 'prov\\_%' ESCAPE '\\'`,
    );
    expect(publicTables.rows).toHaveLength(0);
  });

  it('mirrors exactly the prov-schema table set (13 tables)', async () => {
    ...
    const mirrorNames = Object.values(mirror)
      ...
      .sort();
    expect(MIRROR_TABLE_COUNT).toBe(13);
