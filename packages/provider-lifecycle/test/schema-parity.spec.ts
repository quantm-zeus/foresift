 ...
 * persistence and security packages — see the `sec` precedent for why these
 * tables live outside `public`).
 */
...
describe('prov-schema Drizzle mirror parity with SQL truth (ADR-001)', () => {
  it('applies all five g0_prov migrations over the full proven set', async () => {
    ...
    expect(counts['data']).toBe(7);
    expect(counts['dr']).toBe(5);
    expect(counts['sec']).toBe(4);
  });

  it('adds zero prov tables to the public schema (proven parity contract untouched)', async () => {
  ...
  it('mirrors exactly the prov-schema table set', async () => {
    ...
    const mirrorNames = Object.values(mirror)
      ...
      .sort();
    expect(mirrorNames.length).toBe(EXPECTED_PROV_TABLES);
  ...
  it('matches primary keys on every prov table', async () => {
    ...
    expect(matched).toBe(EXPECTED_PROV_TABLES);
  });
});
