...
export interface LoadOptions extends ValidationOptions { readonly manifestPath: string }
export function loadRequirementManifest(options: LoadOptions): RequirementManifest {
  ...
  if (options.auditPath || options.prdPath || options.sha256sumsPath) validateRequirementManifest({ ...options, manifestData: manifest });
  return manifest;
}
