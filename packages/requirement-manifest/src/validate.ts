...
export const computeTextSha256 = (text: string) =>
  createHash('sha256').update(text, 'utf8').digest('hex');
const fileHash = (file: string) => createHash('sha256').update(readFileSync(file)).digest('hex');
const getManifest = (options: ValidationOptions) =>
  options.manifestData ??
  (JSON.parse(readFileSync(options.manifestPath!, 'utf8')) as RequirementManifest);
  ...
  if (
    sets.some((set) =>
      Object.keys(actual).some(
        (key) => set[key as keyof typeof actual] !== actual[key as keyof typeof actual],
      ),
    )
  )
    throw new RequirementManifestError(Code.COUNT_MISMATCH, 'four-way count disagreement', {
      sets,
    });
...
function validateData(manifest: RequirementManifest, prdText?: string) {
  for (const item of [
    ...
    if (!item.text || computeTextSha256(item.text) !== item.textSha256)
      throw new RequirementManifestError(
        Code.TEXT_HASH_MISMATCH,
        `invalid text hash for ${item.id}`,
        { id: item.id },
      );
  for (const adr of manifest.adrs)
    if (
      ...
      !/^[0-9a-f]{64}$/.test(String(adr.textSha256))
    )
      throw new RequirementManifestError(
        Code.ID_GRAMMAR_INVALID,
        `invalid ADR format for ${adr.id}`,
      );
  if (!checkGlobalIdUniqueness(manifest).isUnique || !checkStableOrdering(manifest).isStable)
    throw new RequirementManifestError(
      Code.ID_REUSE_FORBIDDEN,
      'normative IDs are duplicated or reordered',
    );
  const frIds = new Set(manifest.requirements.map((r) => r.id)),
    acIds = new Set(manifest.acceptanceCriteria.map((a) => a.id)),
    groups = new Set(manifest.dependencyGroups.map((g) => g.id));
  for (const ac of manifest.acceptanceCriteria)
    for (const ref of ac.requirementRefs)
      if (!frIds.has(ref))
        throw new RequirementManifestError(
          Code.DANGLING_REFERENCE,
          `${ac.id} references missing requirement ${ref}`,
        );
  for (const fr of manifest.requirements) {
    if (!groups.has(fr.dependencyGroup))
      throw new RequirementManifestError(
        Code.DANGLING_REFERENCE,
        `${fr.id} references missing dependency group ${fr.dependencyGroup}`,
      );
    for (const ref of fr.acceptanceCriteria)
      if (!acIds.has(ref))
        throw new RequirementManifestError(
          Code.DANGLING_REFERENCE,
          `${fr.id} references missing acceptance criterion ${ref}`,
        );
  }
  ...
  if (!dag.isAcyclic)
    throw new RequirementManifestError(
      ...
      'dependency-group DAG contains a cycle',
      { cycles: dag.cycles },
    );
  if (prdText) {
    ...
    for (const item of [
      ...
      if (!item.line || !lines[item.line - 1]?.includes(item.id))
        throw new RequirementManifestError(
          Code.ANCHOR_UNRESOLVED,
          `PRD line anchor for ${item.id} does not resolve`,
...
export function validateRequirementManifest(options: ValidationOptions) {
  const manifest = getManifest(options);
  const prdText = options.prdPath ? readFileSync(options.prdPath, 'utf8') : undefined;
  validateData(manifest, prdText);
  if (options.auditPath)
    verifyFourWayCountAgreement({ manifestData: manifest, auditPath: options.auditPath });
  const hashes = {
    manifestSha256: options.manifestPath
      ? fileHash(options.manifestPath)
      : computeTextSha256(JSON.stringify(manifest)),
    documentSha256: options.prdPath ? fileHash(options.prdPath) : '',
    auditSha256: options.auditPath ? fileHash(options.auditPath) : '',
  };
  if (options.sha256sumsPath) {
    ...
    for (const [file, hash] of [
      [options.manifestPath, hashes.manifestSha256],
      [options.prdPath, hashes.documentSha256],
      [options.auditPath, hashes.auditSha256],
    ] as const)
      if (file && expected.get(basename(file)) !== hash)
        throw new RequirementManifestError(
          Code.ARTIFACT_HASH_MISMATCH,
          `SHA256SUMS mismatch for ${file}`,
        );
  }
