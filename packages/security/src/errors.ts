...
export const SecErrorCode = {
  ...
  // V7 security-review HIGH H2/H3: a malformed gate request (unknown action,
  // absent actor, or an unvalidatable step-up policy) refuses BEFORE the gate
  // evaluates anything, so an invalid request can never be admitted.
  SEC_ACTION_GATE_INVALID_ACTION: 'SEC_ACTION_GATE_INVALID_ACTION',
  SEC_ACTION_GATE_INVALID_ACTOR: 'SEC_ACTION_GATE_INVALID_ACTOR',
  SEC_ACTION_GATE_INVALID_POLICY: 'SEC_ACTION_GATE_INVALID_POLICY',
  ...
  // V7 security-review HIGH H10: a non-finite/negative admission cost must
  // never silently consume (or fail to consume) budget.
  SEC_ABUSE_COST_INVALID: 'SEC_ABUSE_COST_INVALID',
