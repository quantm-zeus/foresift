/**
 * ADR-001 conformance for the security schema namespace: the Drizzle mirror
 * must match SQL truth. The migrations are the source of truth; this test
 * enumerates every `sec`-schema table in `information_schema` and compares
 * columns, nullability, type classes, and primary keys against the generated
 * mirror (same arrangement as the proven persistence package, scoped to the
...
describe('sec-schema Drizzle mirror parity with SQL truth (ADR-001)', () => {
  it('applies all four g0_sec migrations over the full proven set', async () => {
    const applied = await engine.query<{ id: string }>(
      `SELECT id FROM _foresift_schema_migrations WHERE id LIKE 'g0_sec%' ORDER BY id`,
    );
    ...
    // The proven data/dr sets stay applied alongside — one migration truth.
  ...
  it('mirrors exactly the sec-schema table set', async () => {
    ...
    const mirrorNames = Object.values(mirror)
      ...
      .sort();
    expect(mirrorNames.length).toBe(9);
    ...
    for (const entry of Object.values(mirror)) {
      ...
      const sqlCols = await engine.query<{
        ...
        `SELECT column_name, data_type, is_nullable
         ...
         WHERE table_schema = 'sec' AND table_name = $1
  ...
  it('matches primary keys on every sec table', async () => {
    const sqlPks = await engine.query<{ table_name: string; pk_cols: string[] }>(
      `SELECT tc.table_name,
       ...
       WHERE tc.table_schema = 'sec' AND tc.constraint_type = 'PRIMARY KEY'
