...
describe('packages/shared-schemas: sig schema specifications', () => {
  ...
  const sampleRankingAudit = {
    ...
    sourceDependence: { provider_a: 'INDEPENDENT' },
    adapterScenarioResults: { scenario_1: 'PASSED' },
    ...
    capacityAdmission: { RISK_MONITORING: 'ADMITTED' },
  ...
  it('validates FeatureDefinitionSchema structure and numeric completeness refinement', () => {
    const FeatureDefinitionSchema =
      (sigModule as Record<string, z.ZodTypeAny> | null)?.SigFeatureDefinitionSchema ??
      (sigModule as Record<string, z.ZodTypeAny> | null)?.FeatureDefinitionSchema;
