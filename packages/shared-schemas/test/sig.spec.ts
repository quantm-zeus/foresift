...
describe('packages/shared-schemas: sig schema specifications', () => {
  ...
  const sampleValidLineage = {
    ...
    eventStart: '2026-06-01T11:00:00.000Z',
    eventEnd: '2026-06-01T12:00:00.000Z',
    windowStart: '2026-06-01T11:00:00.000Z',
    ...
    calculatedAt: '2026-06-01T12:00:05.000Z',
    qualityCodes: ['VALID'],
  };
  ...
  const sampleRankingAudit = {
    ...
    protectedAllocations: {
      RISK_MONITORING: '10.0',
    },
    capacityAdmission: {},
    algorithmVersion: 'sig_alg_v1',
    tDecisionReady: '2026-06-01T12:00:00.000Z',
  };
  ...
  const sampleRecheckBudget = {
    ...
    next_check_at: '2026-06-01T12:00:00.000Z',
  ...
  it('validates FeatureDefinitionSchema structure and numeric completeness refinement', () => {
    ...
    if (FeatureDefinitionSchema) {
      ...
      // Invalid capped contribution decimal string fails
      const overCap = { ...sampleValidNumericFeature, cappedContribution: 'invalid_num' };
  ...
  it('validates FeatureLineageSchema sha256 input hashes and timestamp rules', () => {
    ...
    if (FeatureLineageSchema) {
      ...
      // Both inputs empty refused
      const noInputs = { ...sampleValidLineage, inputObservationIds: [], inputEvidenceIds: [] };
      expect(FeatureLineageSchema.safeParse(noInputs).success).toBe(false);
    } else {
  ...
  it('validates CohortSnapshotSchema 7-level fallback hierarchy members', () => {
    ...
    if (CohortSnapshotSchema) {
      ...
      // Malformed decimal string refused
      const invalidDecimal = { ...sampleCohortSnapshot, effectiveSampleSize: 'not_decimal' };
      expect(CohortSnapshotSchema.safeParse(invalidDecimal).success).toBe(false);
    } else {
  ...
  it('validates RecheckBudgetSchema expiry order invariant', () => {
    ...
    if (RecheckBudgetSchema) {
      ...
      const invalidOrder = {
        ...sampleRecheckBudget,
        next_check_at: '2026-06-01T19:00:00.000Z',
        expires_at: '2026-06-01T18:00:00.000Z',
      };
