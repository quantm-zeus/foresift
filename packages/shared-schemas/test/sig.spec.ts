...
describe('packages/shared-schemas: sig schema specifications', () => {
  // Fixture truth follows packages/shared-schemas/src/sig.ts (the schema file
  // is the product contract): decimal quantities are DecimalString (PRD §22
  // money/decimal law), RankingAudit records are native objects, and
  // RecheckBudget uses the PRD §21.2 snake_case field names verbatim.
  ...
  const sampleValidLineage = {
    lineageId: 'lin_001',
    definitionId: 'sig:h1_volume_acceleration',
    definitionVersion: 1,
  ...
  // NB: index.ts renames the SIG FeatureDefinitionSchema to
  ...
  it('validates FeatureDefinitionSchema structure and numeric completeness refinement', () => {
    ...
    if (FeatureDefinitionSchema) {
      ...
      // Capped contribution > 1.0 fails (DecimalString carries the magnitude;
      // the schema's refinement law bounds it — >1 violates the cap contract)
  ...
  it('validates RankingAuditSchema selection probability refinement', () => {
    ...
    if (RankingAuditSchema) {
      ...
      // Selection probability 0 is invalid (must lie in (0,1] as DecimalString)
  ...
  it('validates RecheckBudgetSchema expiry order invariant', () => {
    ...
    if (RecheckBudgetSchema) {
      ...
      // next_check_at >= expires_at refused (PRD §21.2 field names)
      const invalidOrder = {
        ...sampleRecheckBudget,
