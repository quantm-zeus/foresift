...
export function buildBlockedEnvelope(
  ...
  const payload: BlockedStatePayload = {
    ...
    toolVersion: ctx.request.toolVersion ?? ctx.registryEntryName ?? 'unknown',
...
export function buildSuccessEnvelope(ctx: ToolRunContext, now: string): ToolResultEnvelope {
  ...
  const envelope: ToolResultEnvelope = {
    data: {
      observations: observations.map(({ evidenceId, provider, observedAt, availableAt, fetchedAt, fields, qualityCodes: codes }) => ({
        evidenceId,
        provider,
        observedAt,
        availableAt,
      ...
      ...(result.missingCapabilities.length > 0 ? { missingCapabilities: [...result.missingCapabilities] } : {}),
      ...(conflictRefs.length > 0 ? { conflicts: conflictRefs } : {}),
    },
    meta: {
      toolName: ctx.request.toolName,
      toolVersion: ctx.request.toolVersion ?? ctx.registryEntryName ?? 'unknown',
      ...(ctx.route !== undefined ? { provider: ctx.route.provider, operation: ctx.route.operation } : {}),
      evidenceIds:
      ...
      ...(observations.length > 0 ? { availableAt: minBy(observations, (o) => o.availableAt) } : {}),
      fetchedAt: maxBy(observations, (o) => o.fetchedAt) ?? now,
      cache: (ctx.cacheOutcome ?? (ctx.servedFromCache ? 'HIT_FRESH' : 'REFRESHED')) as CacheOutcome,
      ...
      quota: {
        quotaModel: (ctx.estimate?.quotaModel ?? 'UNKNOWN_CONFIGURABLE') as QuotaModel,
        reservationState: (ctx.quotaState ?? (ctx.servedFromCache ? 'PENDING' : 'COMMITTED')) as ReservationState,
        estimatedUnits: ctx.estimate?.estimatedUnits ?? null,
...
export async function makeEnvelopeStageRun(
  ...
  if (ctx.servedFromCache && ctx.result === undefined) {
    ...
    if (newestFetched !== undefined) {
      ctx.cachedFreshnessSeconds = Math.max(0, Math.floor((Date.parse(deps.now()) - Date.parse(newestFetched)) / 1000));
    }
  }
  const now = deps.now();
  const envelope =
    ctx.blocked !== undefined
      ? buildBlockedEnvelope(ctx, now)
      : buildSuccessEnvelope(ctx, now);
