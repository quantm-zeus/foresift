 ...
 */
import type { NormalizedObservation, NormalizedResult, PayloadNormalizer } from './provider-contract.ts';
...
export const normalizeRawPayload: PayloadNormalizer = (raw, context) => {
  ...
  const observations: NormalizedObservation[] = payload.observations.map(
    (entry: unknown, index: number): NormalizedObservation => {
      if (entry === null || typeof entry !== 'object') fail(`observation ${String(index)} is not an object`);
      ...
      const observedAt = isoTimestamp(obs.observedAt, `observations[${index}].observedAt`, undefined);
      const availableAt = isoTimestamp(
        obs.availableAt,
        `observations[${index}].availableAt`,
        context.fetchedAt,
      );
      const fetchedAt = isoTimestamp(obs.fetchedAt, `observations[${index}].fetchedAt`, context.fetchedAt);
      return {
        ...
        availableAt,
        fetchedAt,
        fields: { ...(identity !== '' ? { identity } : {}), ...normalizedFields(obs.fields, `observations[${index}].fields`) },
...
export function validateNormalizedInvariants(
  ...
  for (const observation of result.observations) {
    ...
    if (available > fetched + 60_000) {
      problems.push(`${observation.evidenceId}: availableAt exceeds fetchedAt beyond skew tolerance`);
    }
