...
export interface AcquisitionStageDeps {
   ...
   */
  readonly probeAssignment?: ((ctx: ToolRunContext) => {
...
export function makePersistRequestedStage(deps: AcquisitionStageDeps) {
  return async (ctx: ToolRunContext): Promise<void> => {
    const decisionId = `dec-${ctx.runId}`;
    ...
    const existing = await deps.engine.query<{ decision_id: string; state: string }>(
      'SELECT decision_id, state FROM evidence_acquisition_decisions WHERE decision_id = $1',
      [decisionId],
    );
    ...
    await recordAcquisitionDecision(deps.engine, {
      ...
      evidenceFamily: ctx.route?.operation ?? ctx.request.toolName,
