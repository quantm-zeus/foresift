...
export type PipelineAuditClass = 'TOOL_RESOURCE_ACCESS' | 'PROVIDER_COLLECTOR_ACCESS' | 'BLOCKED_OPERATION';
...
export function makeAuditStage(deps: AuditStageDeps) {
  return async (ctx: ToolRunContext): Promise<void> => {
    ...
    const payload = {
      ...
      actor: sha256Text(ctx.actor?.actorId ?? 'anonymous').slice(0, 16),
      toolName: ctx.request.toolName,
      toolVersion: ctx.request.toolVersion ?? ctx.registryEntryName ?? '*',
    ...
    const record = await deps.auditChain.append({
      occurredAt: deps.now(),
      actor: 'tool-core-pipeline',
      actionClass: auditClassFor(ctx),
      subject: `${ctx.request.toolName}@${payload.toolVersion}:${ctx.runId}`,
      payload,
    });
    ctx.audited = true;
    ctx.auditRecord = { seq: record.seq, entryHash: record.entryHash, auditClass: auditClassFor(ctx) };
  };
}
