...
export function makeAuthenticateStage(deps: AuthnStageDeps) {
  return async (ctx: ToolRunContext): Promise<void> => {
    ...
    if (!identity.actorId || !identity.profileId) {
      throw new ForesiftError(
        'AUTHENTICATION_REFUSED',
        'authentication primitive returned an incomplete identity',
        {},
      );
    }
    ctx.actor = identity;
...
export function makeAuthorizeStage(deps: AuthorizeStageDeps) {
  return async (ctx: ToolRunContext): Promise<void> => {
    ...
    if (route === undefined) {
      block(ctx, 'CAPABILITY_UNAVAILABLE', `tool ${req.toolName} has no composed operation route`, 'AUTHORIZE');
      return;
    }
    ctx.route = route;
    ...
    if (!isAdmissibleActionClass(entry.metadata.actionClass)) {
      block(ctx, 'RIGHTS_BLOCKED', `action class ${entry.metadata.actionClass} is not admissible`, 'AUTHORIZE');
      return;
    }
    ...
    const rights = await deps.licenseSource.verdict({
