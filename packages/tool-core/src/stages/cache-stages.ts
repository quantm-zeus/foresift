...
function serveFromCache(ctx: ToolRunContext, outcome: 'HIT_FRESH' | 'HIT_STALE' | 'MEMO_HIT'): void {
...
export function makeCacheKeyStage(deps: CacheStagesDeps) {
  return async (ctx: ToolRunContext): Promise<void> => {
    ...
    const components: CacheKeyComponents = {
      ...
      fieldProjection: [
        ...(ctx.request.fieldProjection ?? route.fieldProjection),
      ],
...
export function makeCacheLookupStages(deps: CacheStagesDeps) {
  // The chain evaluates memo → FRESH → stale internally in one pass; each
  // stage below interprets ITS tier's verdict so the §16.2 sequence stays
  // observable in the journal.
  let lookupRan = false;

  const runLookup = async (ctx: ToolRunContext): Promise<void> => {
    if (lookupRan) return;
    lookupRan = true;
    const verdict = await deps.chain.lookup({
      components: ctx.cacheComponents!,
      holderMode: ctx.request.holderMode,
    });
    if (verdict.outcome !== 'MISS') {
      ctx.cacheOutcome = verdict.outcome === 'MEMO_HIT' ? 'HIT_FRESH' : verdict.outcome;
  ...
  return {
    ...
    memoStage: async (ctx: ToolRunContext): Promise<void> => {
      if (resolvedFromCache(ctx)) return;
      await runLookup(ctx);
      if (ctx.cacheOutcome === 'HIT_FRESH') serveFromCache(ctx, 'MEMO_HIT');
    },
    /** Stage 8 — CHECK_FRESH_CACHE. */
    freshStage: async (ctx: ToolRunContext): Promise<void> => {
      if (resolvedFromCache(ctx)) return;
      await runLookup(ctx);
      if (ctx.cacheOutcome === 'HIT_FRESH') serveFromCache(ctx, 'HIT_FRESH');
    },
    /** Stage 9 — CHECK_ACCEPTABLE_STALE_CACHE_IF_ALLOWED. */
    staleStage: async (ctx: ToolRunContext): Promise<void> => {
      if (resolvedFromCache(ctx)) return;
      await runLookup(ctx);
      if (ctx.cacheOutcome === 'HIT_STALE') serveFromCache(ctx, 'HIT_STALE');
    },
  };
}
