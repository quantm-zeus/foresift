...
export function classifyDispatchFailure(error: unknown): {
  ...
  if (error instanceof EgressRefusedError || /egress|allowlist|dns|connect|unreachable/i.test(message)) {
...
export function makeDispatchStage(deps: DispatchStageDeps) {
  return async (ctx: ToolRunContext): Promise<void> => {
    ...
    const findings = deps.executionGate({
      ...
      schemaJsonText: JSON.stringify(ctx.canonicalInput ?? {}),
    });
    if (findings.length > 0 || ctx.actionClass === 'PROHIBITED_FINANCIAL') {
      block(
        ctx,
        'RIGHTS_BLOCKED',
    ...
    if (deps.egressGuard === undefined) {
      block(ctx, 'PROVIDER_UNAVAILABLE', 'EGRESS_UNCONFIGURED: no egress guard is composed', 'DISPATCH');
      return;
    }
    const decision = await deps.egressGuard.authorize(route.endpointUrl, route.egressPlane);
    if (decision.decision !== 'ALLOW') {
      throw new EgressRefusedError(`${decision.reason}: ${decision.detail}`);
    }
    ...
    try {
      ...
      const deadline = new Promise<never>((_, reject) => {
        timer = setTimeout(() => reject(new DeadlineExceededError(route.deadlineMs)), route.deadlineMs);
      });
...
const ADMITTED_CONTENT_TYPE_PREFIX = 'application/json';
...
export function makeRawValidateStage() {
  return async (ctx: ToolRunContext): Promise<void> => {
    ...
    const route = ctx.route!;
    if (!raw.contentType.split(';')[0]!.trim().toLowerCase().startsWith(ADMITTED_CONTENT_TYPE_PREFIX)) {
      block(
        ctx,
        'INVALID_RESPONSE',
        `CONTENT_TYPE_REFUSED: ${raw.contentType} (admitted prefix: ${ADMITTED_CONTENT_TYPE_PREFIX})`,
        'RAW_VALIDATE',
      );
      return;
    }
