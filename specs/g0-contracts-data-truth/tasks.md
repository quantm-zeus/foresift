...
- [x] T066 Wire `PRECISION_RETAINING_TIMESTAMP_PARSERS` into the production
      ...
      Resolved 2026-08-23 by landing the enforcement mechanism inside the
      package's `packages/persistence/**` scope: `createProductionPgPool(pg,
config)` is now the only sanctioned production-pool constructor; it
      applies `wirePrecisionRetainingTimestampParsers`
      (`types.setTypeParser`, OIDs 1114/1184 from
      `PRECISION_RETAINING_TIMESTAMP_PARSERS`) before the first Pool exists.
      Proven by `packages/persistence/test/pg-pool-wiring.spec.ts`
      (registration-before-construction ordering, OID mapping, parser
      precision/fail-closed behavior, idempotent re-wiring). The
      deployment-facing package must obtain all production pools through
      this factory; parser activation is then automatic at first
      construction, so the wiring can no longer be forgotten. Remaining
      obligation (config provisioning, calling the factory) is recorded in
      `out-of-scope-notes.md` §8.
