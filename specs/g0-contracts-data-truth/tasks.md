...
- [ ] T004 Implement `packages/domain/src/chain.ts` + `address.ts`: CAIP-2 chain-id
  ...
  Traces: FR-DATA-001.
- [ ] T005 Implement `packages/domain/src/quantity.ts`: raw-integer amounts + decimals,
...
- [ ] T006 Implement `packages/domain/src/timestamps.ts` + `availability.ts`: §13.1
  ...
  Traces: FR-DATA-003.
- [ ] T007 Implement `packages/domain/src/quality.ts` + `acquisition.ts`: full §13.9
...
- [ ] T008 [P] Implement `packages/domain/src/asset.ts`, `pool.ts`, `launch.ts`:
...
- [ ] T009 [P] Implement `packages/domain/src/source.ts`: SourceIdentity fields
...
- [ ] T010 Implement `packages/domain/src/feature.ts` + `recovery.ts` + `errors.ts`:
...
- [ ] T011 Unit tests for domain modules incl. property test: replay predicate is
