...
registry), and `tests/telemetry-catalog.spec.ts` (central telemetry parity
suite) are extended by this package even though they sit outside the listed
writeScopes. The ADR-001 mirror duty follows the landed precedent of
g1-data-truth-extensions (#175) and g1-execution-simulation (#208).
...
- [ ] T001 Create `packages/domain/src/sig.ts`: `FunnelStage` (21 §20.1 stages,
      ...
      proven `TradabilityVerdict` TRADABLE member is the only pass). Add
      colocated `packages/domain/test/` fail-closed vocabulary + law
      truth-table tests. Traces: FR-SIG-001, FR-SIG-002, FR-SIG-003, FR-SIG-004,
      FR-SIG-005, FR-SIG-006, FR-SIG-009, AC-136, AC-154.
- [ ] T002 Extend `packages/domain/src/index.ts` exports for the new sig
      module. Traces: FR-SIG-001…006, FR-SIG-009.
- [ ] T003 Create `packages/shared-schemas/src/sig.ts`: Zod schemas for
      ...
      layer. Extend `packages/shared-schemas/src/index.ts` exports. Add
...
- [ ] T004 Create `migrations/g1_sig_0001_feature_registry.sql`:
      ...
      AC-136.
- [ ] T005 Create `migrations/g1_sig_0002_funnel_vectors_ranking.sql`:
      ...
      AC-192, AC-193.
- [ ] T006 Create `migrations/g1_sig_0003_lifecycle_rechecks.sql`:
...
- [ ] T007 Extend `packages/persistence/src/migrator.ts`
      MIGRATION_FAMILIES with the `sig` family (fail-closed pattern extension)
      AND extend the central expected-script registry
      `packages/persistence/test/migrator.spec.ts` with
      `g1_sig_0001_feature_registry`, `g1_sig_0002_funnel_vectors_ranking`,
      `g1_sig_0003_lifecycle_rechecks` (lexicographic, checksum-pinned) —
      the plan-sanctioned central-registry scope exception (plan-level
      decision 1; ADR-0019/0022 duty; the registry suite must be updated in
      the same package or the task-graph guard refuses the build). Traces:
      FR-SIG-001…006, FR-SIG-009.
- [ ] T008 Extend `packages/persistence/src/generated/schema.ts` (the
      hand-maintained ADR-001 Drizzle mirror) with the sig.* tables from
      g1_sig_0001…0003 so the schema-parity gate
      (`packages/persistence/test/schema-parity.spec.ts`) stays green — the
      plan-sanctioned mirror catch-up scope exception (exact precedent:
      g1-data-truth-extensions #175, g1-execution-simulation #208).
      Traces: FR-SIG-001, FR-SIG-002, FR-SIG-003, FR-SIG-004, FR-SIG-005,
      FR-SIG-006, FR-SIG-009.

## Phase 3 — Feature registry, stability, and Appendix H baseline computation (blocks vectors and selection)

- [ ] T009 Scaffold `packages/signal-intelligence` (package.json
...
- [ ] T010 Implement `packages/signal-intelligence/src/registry.ts` +
      ...
      `supportsPopulationClaim` seam). Colocated tests: registration
      drift refusal, lineage completeness, unavailable-at-decision-time
      refusal. Traces: FR-SIG-001, FR-SIG-009, AC-021.
- [ ] T011 Implement `packages/signal-intelligence/src/stability.ts` +
      ...
      upstream data degrade explicitly). Colocated tests: fallback-level
      truth table (all seven reachable and deterministic), shrinkage bounds,
      capped-contribution clamp, low-sample warnings. Traces: FR-SIG-009,
      FR-SIG-001, AC-136.
- [ ] T012 Implement `packages/signal-intelligence/src/baseline-features.ts`:
      ...
      proof), H.12 data coverage. Colocated tests: golden vectors per formula,
      boundedness properties (imbalance ∈ [−1,1], entropy ∈ [0,1]), window
      boundary fixtures, dedup threshold fixtures. Traces: FR-SIG-001,
      FR-SIG-009, AC-136, AC-020, AC-022, AC-023.
- [ ] T013 Implement `packages/signal-intelligence/src/robust-baseline.ts`:
      ...
      insufficient. Colocated tests: emergence fixtures, no-promotion-below-
      minimum, CUSUM drift vectors, missing-bucket handling. Traces:
      FR-SIG-009, FR-SIG-002, AC-136.

## Phase 4 — Funnel, vectors, and deterministic selection (blocks diversity/lifecycle)

- [ ] T014 Implement `packages/signal-intelligence/src/funnel.ts` +
      ...
      law (no vector set collapses to one scalar). Colocated tests: gate
      reason-coding truth table, vector null-honesty, unknown-blocking,
      tradability consumption. Traces: FR-SIG-002, AC-020, AC-136.
- [ ] T015 Implement `packages/signal-intelligence/src/selection.ts`:
      ...
      byte-identical audit). Colocated tests: reproducibility
      (byte-identical rerun), step-indexed positive/negative cases, sort-key
      a–g ordering fixtures, frozen-universe drift refusal. Traces: FR-SIG-003,
      FR-SIG-002, AC-154, AC-190.
- [ ] T016 Implement `packages/signal-intelligence/src/challenger-seam.ts`:
      ...
      probability. Colocated tests: pre-proof ordering-input refusal, override
      refusal truth table, drift → automatic degradation, tie-break-only
      effect. Traces: FR-SIG-003, AC-154.

## Phase 5 — Diversity, exploration, lifecycle, rechecks (blocks AC suites)

- [ ] T017 Implement `packages/signal-intelligence/src/diversity.ts` +
      ...
      weighted claims by pure predicate). Colocated tests: constraint
      ...
      audit trail. Traces: FR-SIG-004, AC-192, AC-193, AC-190.
- [ ] T018 Implement `packages/signal-intelligence/src/lifecycle.ts`:
...
- [ ] T019 Implement `packages/signal-intelligence/src/rechecks.ts` +
      ...
      no-LLM import assertion (structural test asserting the package imports
      no model-provider/agent surface — no LLM anywhere in the deterministic
      path, Appendix I step 14), prohibited-capability scanner run. Colocated
      tests: budget-exhaustion state machine, starvation/expiry/backoff
...
- [ ] T020 Author `tests/fixtures/sig/feature-definitions.json` +
...
- [ ] T021 Author `tests/fixtures/sig/cohort-fallback.json` +
...
- [ ] T022 Author `tests/fixtures/sig/funnel-selection.json` +
...
- [ ] T023 Author `tests/acceptance/AC-154.spec.ts` +
...
- [ ] T024 Author `tests/acceptance/AC-190.spec.ts` +
...
- [ ] T025 Author `tests/acceptance/AC-191.spec.ts` +
...
- [ ] T026 Author `tests/acceptance/AC-192.spec.ts` +
      ...
      FR-SIG-004, AC-192.
- [ ] T027 Author `tests/acceptance/AC-193.spec.ts` +
      ...
      AC-193.
- [ ] T028 Extend `tests/acceptance/AC-020.spec.ts` +
...
- [ ] T029 Extend `tests/acceptance/AC-021.spec.ts` +
...
- [ ] T030 Extend `tests/acceptance/AC-022.spec.ts` +
...
- [ ] T031 Extend `tests/acceptance/AC-023.spec.ts` +
...
- [ ] T032 Extend `tests/acceptance/AC-136.spec.ts` +
...
- [ ] T033 Create `telemetry/sig.catalog.json` (DECLARATIVE_CONTRACT_ONLY
      ...
      (FR-SIG-006). Extend `tests/telemetry-catalog.spec.ts` — the
      plan-sanctioned central-parity scope exception (plan-level decision 4)
      — pinning the new catalog to the authoritative schemas. Traces:
      FR-SIG-001, FR-SIG-002, FR-SIG-003, FR-SIG-004, FR-SIG-005, FR-SIG-006,
      FR-SIG-009.
