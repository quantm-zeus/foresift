...
- **Scope**: every predicted write lands inside writeScopes except the
  plan-sanctioned exceptions named by exact path (T007 migrator family
  extension, product-owned; T008 ADR-001 mirror catch-up, product-owned;
  T039 central migration registry suite and T040 central telemetry parity
  suite, both test-owned; T035 mechanical manifest regen per ADR-0020
  coordinator duty), consistent with the live g1-data-truth (#175) /
  g1-execution-simulation (#208) precedent. Ownership routing per the
  2026-09-07 admission law: all test/fixture/spec authorship lives in
  test-owned tasks (T020-T032 authorship work, T037-T041), implementation
  lanes carry product code only (T001-T019, T033); parallelism markers per
  the 2026-09-08 directive: [P] on write-disjoint tasks, exactly one
  six-vocabulary [serial-reason] on each truly-serial product task
  (ORDERED_MIGRATION for the g1_sig migration trio; SEMANTIC_DEPENDENCY for
  the deterministic pipeline chain and the export scaffold task).
