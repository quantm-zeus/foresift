...
// The red target advances down the manifest as packages land (originally
// g0-tool-core, whose own landing expired that pin).
...
describe('V2 structured gate manifest — REAL red package gate (spec §9)', () => {
  ...
  itE2e(
    ...
    (ctx) => {
      if (!unlanded)
        return ctx.skip('every declared verification target exists — no deterministic red remains');
      ...
      // expectation does too: the FAIL row must be a declared command of the
      // derived red package, and every DECLARED command before it must have
      // had a satisfied `test -d` target (absent targets fail
      // deterministically, so a satisfied predecessor proves the stop was at
      // the first unsatisfied check).
