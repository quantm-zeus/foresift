...
const stubArchon = () => {
  ...
  const ghScript = [
    ...
    '  "pr merge"*)',
    "    STATE_BRANCH=$(git for-each-ref --format='%(refname:short)' refs/heads/state/chore/ | head -n1)",
    '    if [ -n "$STATE_BRANCH" ]; then',
    '      git push origin "$STATE_BRANCH:refs/heads/main" >/dev/null 2>&1 || true',
    '      touch "/tmp/gh_stub_merged_${STATE_BRANCH//\\//_}"',
    '    fi',
    '    printf \'{"merged":true}\\n\'',
    '    ;;',
    '  "pr view"*)',
    '    MAIN_SHA=$(git rev-parse refs/remotes/origin/main 2>/dev/null || git rev-parse origin/main 2>/dev/null || git rev-parse HEAD 2>/dev/null || echo "0000000000000000000000000000000000000000")',
    "    STATE_BRANCH=$(git for-each-ref --format='%(refname:short)' refs/heads/state/chore/ | head -n1)",
    '    STATE_SHA=$(git rev-parse "origin/$STATE_BRANCH" 2>/dev/null || git rev-parse "$STATE_BRANCH" 2>/dev/null || echo "$MAIN_SHA")',
    '    STATE_STATUS="OPEN"',
    '    if [ -n "$STATE_BRANCH" ] && [ -f "/tmp/gh_stub_merged_${STATE_BRANCH//\\//_}" ]; then STATE_STATUS="MERGED"; fi',
    '    printf \'{"state":"%s","headRefOid":"%s","baseRefName":"main","mergeCommit":{"oid":"%s"}}\\n\' "$STATE_STATUS" "$STATE_SHA" "$MAIN_SHA"',
    '    ;;',
    '  "run list"*) printf \'[]\' ;;',
    '  "run view"*) printf \'\' ;;',
    '  *) printf \'{"ok":true}\\n\' ;;',
