...
describe("AC-132 negative: System address refusal and quality code degradation", () => {
  ...
  it("refuses exclusion on pending review or rejected review state", () => {
    const fixture = JSON.parse(readFileSync(SYSTEM_REGISTRY_FIXTURE, "utf8"));
    const pendingCase = fixture.entries.find((e: any) => e.expectedExclusionDecision === "REFUSAL_PENDING_REVIEW");
    const rejectedCase = fixture.entries.find((e: any) => e.expectedExclusionDecision === "REFUSAL_REJECTED");
  ...
  it("refuses exclusion on unknown infrastructure role", () => {
    const fixture = JSON.parse(readFileSync(SYSTEM_REGISTRY_FIXTURE, "utf8"));
    const unknownRoleCase = fixture.entries.find((e: any) => e.expectedExclusionDecision === "REFUSAL_UNKNOWN_ROLE");
